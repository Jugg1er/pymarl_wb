...
class VQLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        for t in range(batch.max_seq_length):
            agent_outs, vae_loss, recon_s = self.mac.forward(batch, t=t)
        ...
        with th.no_grad():
            ...
            self.target_mac.vae.train()
            target_mac_out = []
            self.target_mac.init_hidden(batch.batch_size)
            for t in range(batch.max_seq_length):
                target_agent_outs, _, target_recon_s = self.target_mac.forward(batch, t=t)
        ...
        loss = masked_td_error.sum() / mask.sum() + vae_losses
