...
class DOTMAC:
    def __init__(self, scheme, groups, args):
        ...
        self.mlp = nn.Sequential(nn.Linear(args.rnn_hidden_dim, args.rnn_hidden_dim), nn.ReLU(), nn.Linear(args.rnn_hidden_dim, args.n_agents))
        # self.beta = args.beta
        self.beta = th.Tensor([0]).cuda()
    ...
    def forward(self, ep_batch, t, test_mode=False):
        ...
        ade_outs = None
        if not test_mode:
            ade_outs = self.mlp(self.hidden_states.clone()).view(ep_batch.batch_size, self.n_agents, -1)
            ade_outs = F.log_softmax(ade_outs, dim=-1)
