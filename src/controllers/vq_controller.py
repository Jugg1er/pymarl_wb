...
class VQMAC:
    ...
    def forward(self, ep_batch, t, test_mode=False):
        ...
        recon_s, z_e, role_emb = self.vae(agent_inputs)
        vae_loss = self.vae.loss_function(state, recon_s, z_e, role_emb)
        agent_outs, self.hidden_states = self.agent(agent_inputs, self.hidden_states, role_emb)
        ...
        return agent_outs.view(ep_batch.batch_size, self.n_agents, -1), vae_loss, recon_s

    def init_hidden(self, batch_size):
        self.hidden_states = self.agent.init_hidden().unsqueeze(0).expand(batch_size, self.n_agents, -1)  # bav

    # def parameters(self):
    #     return self.agent.parameters()#, self.vae.parameters()

    def load_state(self, other_mac):
        self.agent.load_state_dict(other_mac.agent.state_dict())
        self.vae.load_state_dict(other_mac.vae.state_dict())
    ...
    def _build_vae(self, input_shape):
        self.vae = vae_REGISTRY[self.args.vae](input_shape, self.state_dim, self.args)
